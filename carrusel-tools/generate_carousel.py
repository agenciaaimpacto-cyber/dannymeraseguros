#!/usr/bin/env python3
"""
Genera un carrusel de imagenes (1080x1350, estilo Danny Mera Seguros: fondo
verde azulado oscuro, texto blanco/dorado bold) a partir de un archivo JSON
de slides.
...
{
  "slides": [
    {"type": "hook", "tag": "SABIAS QUE...", "text": "Tu credito trae un seguro que **no elegiste tu**"},
    {"type": "text", "lines": ["Viene por convenio entre el banco", "**y una aseguradora especifica.**"], "sub": "No necesariamente la mas barata para ti."},
    {"type": "stat", "number": "10", "unit": "dias", "text": "es el plazo maximo que tiene la aseguradora para **devolverte tu plata**"},
    {"type": "proof", "tag": "LEY 21.236", "text": "Tienes derecho a **portar** tu seguro de desgravamen a una compania mas barata", "source": "Fuente: CMF Educa / Ley de Portabilidad Financiera"},
    {"type": "cta", "text": "Quieres saber cuanto puedes **recuperar tu?**", "button": "Escribeme por WhatsApp ->", "sub": "Revisamos tu caso sin costo."}
  ]
}

Marcado: **palabra** = resaltado en dorado. El resto va en blanco (o gris para
...
W, H = 1080, 1350
PAD = 90
BG = (15, 61, 62)          # teal-dark
GOLD = (201, 162, 39)      # gold
WHITE = (250, 250, 247)    # paper
DIM = (200, 214, 210)
GRAY_LABEL = (140, 168, 163)
GRAY_SOURCE = (120, 148, 143)
...
def tokenize(text, base_color):
    """'hola **mundo** feliz' -> [('hola', base_color), ('mundo', GOLD), ('feliz', base_color)]"""
    tokens = []
    for m in TOKEN_RE.finditer(text):
        if m.group(1) is not None:
            for w in m.group(1).split():
                tokens.append((w, GOLD))
        else:
...
def render_hook(slide):
    ...
    if slide.get("tag"):
        ...
        d.text((PAD, y), slide["tag"].upper(), font=f, fill=GOLD)
        y += 70
    f = load_black(76)
...
def render_stat(slide):
    ...
    fnum = load_black(220)
    y = 480
    d.text((PAD, y), slide["number"], font=fnum, fill=GOLD)
    if slide.get("unit"):
        numw = d.textlength(slide["number"], font=fnum)
        funit = load_black(100)
        d.text((PAD + numw + 10, y + 100), slide["unit"], font=funit, fill=GOLD)
    y += 280
