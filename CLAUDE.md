...
- **Incidente detectado el 28 de agosto de 2026 — corregido, pero ojo con esto en corridas futuras:** las corridas del 26 y 27 de agosto habían hecho `git commit` correctamente pero el commit quedó en un HEAD separado de la rama `main` (probablemente por un `git checkout` a un commit específico al inicio de esa sesión, en vez de quedarse parada en `main`), así que el `git push` de esos días nunca actualizó `origin/main` — el sitio en vivo se quedó 2 días sin el despacho ni los carruseles nuevos, aunque el repo local sí tenía los commits. Se detectó porque `git log --oneline` (que revisa el historial local) mostraba los commits, pero `git log --oneline origin/main` no — hay que comparar ambos, no confiar solo en el local. Se corrigió haciendo fast-forward de `main` a esos commits y pusheando los 3 días de una vez. **Para evitar que se repita:** antes de terminar cualquier corrida, verificar con `git status` que dice "On branch main" (no "HEAD detached") antes de commitear, y después del push confirmar con `git log --oneline origin/main -1` que el commit de hoy quedó reflejado en el remoto, no solo en el log local.
