...
- **Incidente de HEAD separado — pasó DOS VECES, la "corrección" del 28 de agosto no alcanzó a evitar que se repitiera:** las corridas del 26 y 27 de agosto quedaron con el commit en un HEAD separado de `main` (detectado y corregido el 28 de agosto, ver nota original más abajo). A pesar de esa corrección y de la regla agregada ("verificar `git status` antes de commitear"), **volvió a pasar en las corridas del 27 y 28 de agosto**: la sesión que arrancó el 29 de agosto encontró de nuevo `HEAD detached from refs/heads/main`, con 4 commits (26, 27, 28 de agosto + el commit que documentaba el incidente anterior) nunca fusionados a `main` ni pusheados a `origin/main` — es decir, la corrección del 28 de agosto tampoco quedó push-eada correctamente, se quedó atrapada en el mismo problema que describía. Se corrigió de nuevo el 29 de agosto: `git checkout main && git merge --ff-only <hash>` seguido de `git push`, confirmando después con `git log --oneline origin/main` que los 4 commits llegaron al remoto. **Causa raíz probable, todavía sin confirmar:** cada sesión programada de esta rutina parece arrancar en un estado de git que no es "parado en main" (posiblemente el entorno de la nube hace un checkout a un commit específico al iniciar el contenedor, en vez de a la rama). Si esto se repite una tercera vez, vale la pena revisar la configuración de la rutina en sí (cómo clona/inicializa el repo al arrancar cada sesión) en vez de seguir confiando en que cada corrida se acuerde de revisar `git status` al final — un chequeo manual al final de la corrida claramente no es suficiente para prevenirlo, porque la corrida que sí hizo la corrección también cayó en el mismo estado. **Regla que se mantiene, pero no basta por sí sola:** antes de commitear, verificar con `git status` que dice "On branch main" (no "HEAD detached"); si dice detached, hacer `git checkout main && git merge --ff-only <HEAD-detached>` antes de seguir. Después del push, confirmar con `git log --oneline origin/main -1` que el commit de hoy quedó reflejado en el remoto, no solo en el log local.
- **Tercera ocurrencia, confirmada el 30 de agosto — la corrección del 29 de agosto tampoco evitó que se repitiera:** la sesión que arrancó el 30 de agosto encontró de nuevo `HEAD detached from refs/heads/main`, con 5 commits (26, 27, 28, 29 de agosto + el commit que documentaba la segunda ocurrencia) nunca fusionados a `main` ni pusheados a `origin/main` — `origin/main` seguía parado en el commit del 25 de agosto hasta este momento. Se corrigió de la misma forma (`git checkout main && git merge --ff-only 07ff2fc && git push`), confirmado con `git fetch` + `git log --oneline origin/main` que los 6 commits llegaron al remoto. Esto ya pasó tres veces seguidas a pesar de la regla de verificar `git status` — la propia corrida que documentó la segunda ocurrencia volvió a quedar atrapada en HEAD separado. El chequeo manual al final de cada corrida no es una solución, es un parche que se repite cada vez. **Pendiente real de investigar (no delegar a la próxima corrida):** revisar la configuración de la rutina en `claude.ai/code/routines/trig_01LVw2LmF7pPV7TwSm6JuiEF` — específicamente cómo el entorno de la nube clona/inicializa el repo al arrancar cada sesión (¿checkout a un commit fijo en vez de a la rama `main`?).
- **Cuarta ocurrencia, confirmada el 31 de agosto — pero con un dato nuevo importante que aclara la causa raíz:** la sesión que arrancó el 31 de agosto encontró otra vez `HEAD detached from refs/heads/main`. La diferencia esta vez: al hacer `git fetch origin main`, `origin/main` ya estaba al día con el commit del 30 de agosto (`bedc0ef`) — es decir, **el push de la corrida anterior sí llegó bien al remoto**, el problema es exclusivamente que la rama local `main` de la sesión nueva no se actualiza ni se usa como punto de partida. Esto confirma bastante mejor la hipótesis: cada sesión nueva arranca el contenedor con un `checkout` a un commit específico (probablemente el que estaba fijado al momento de crear/clonar el entorno), no a la rama `main`, y por eso cada sesión ve `HEAD detached` desde el primer momento aunque el remoto esté perfectamente sincronizado. No hay pérdida de contenido en esta ocurrencia. Se corrigió igual que las anteriores (`git checkout main && git merge --ff-only origin/main`, ya que esta vez `origin/main` iba adelante de la rama local `main`, no al revés) y se confirmó con `git fetch` + `git log --oneline origin/main` que el commit del 31 de agosto llegó al remoto. **Sigue pendiente sin resolver:** revisar la configuración de la rutina (cómo inicializa el repo al arrancar cada sesión) — ya son cuatro corridas seguidas con el mismo síntoma, el chequeo manual funciona como parche pero no ataca la causa.
- **Quinta ocurrencia, confirmada el 1 de septiembre — mismo síntoma, sin pérdida de contenido:** la sesión que arrancó el 1 de septiembre encontró de nuevo `HEAD detached from refs/heads/main`, esta vez ya parado exactamente en el mismo commit que `origin/main` (`bbef5d2`, el commit del 31 de agosto) — o sea que, igual que la cuarta ocurrencia, el remoto estaba perfectamente al día y el único problema fue que la sesión arrancó sin la rama local `main` apuntando ahí. Se corrigió con `git checkout main && git merge --ff-only bbef5d2` antes de tocar ningún archivo, y se confirmó con `git fetch` + `git log --oneline origin/main` que el commit del 1 de septiembre llegó al remoto después del push. Van cinco corridas seguidas con el mismo síntoma. El pendiente sigue siendo el mismo y ya no vale la pena repetirlo cada día: **revisar la configuración de la rutina en `claude.ai/code/routines/trig_01LVw2LmF7pPV7TwSm6JuiEF`** (cómo el entorno de la nube inicializa el repo al arrancar cada sesión) es la única forma de atacar la causa en vez de seguir parchando; el chequeo manual seguirá aplicándose mientras tanto porque funciona, pero no es una solución.
- **Sexta ocurrencia, confirmada el 2 de septiembre — mismo síntoma, sin pérdida de contenido:** la sesión que arrancó el 2 de septiembre encontró de nuevo `HEAD detached from refs/heads/main`, otra vez parado exactamente en el commit que ya estaba al día en `origin/main` (`bd42f23`, el commit que documentaba la quinta ocurrencia del 1 de septiembre). El remoto estaba perfectamente sincronizado, el único problema fue —igual que la cuarta y quinta vez— que la rama local `main` de la sesión nueva no apuntaba ahí. Se corrigió de la misma forma antes de tocar ningún archivo (`git checkout main && git merge --ff-only bd42f23`) y se confirmó con `git fetch` + `git log --oneline origin/main` que el commit del 2 de septiembre llegó al remoto después del push. Van seis corridas seguidas con el mismo síntoma, todas resueltas sin pérdida de contenido gracias al chequeo manual — pero eso mismo confirma que el chequeo es un parche estable, no una solución, y que el pendiente de fondo (revisar cómo `claude.ai/code/routines/trig_01LVw2LmF7pPV7TwSm6JuiEF` inicializa el repo al arrancar cada sesión) sigue sin investigarse. No hace falta seguir agregando una entrada por cada ocurrencia si el patrón se repite igual — lo que realmente falta es que alguien revise la configuración de la rutina directamente.
